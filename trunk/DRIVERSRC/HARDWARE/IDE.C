...
uint8_t _IDE_readPort(bool secondary, uint8_t reg)
{
	...
	if (reg > 0x07 && reg < 0x0C)
		_IDE_writePort(secondary, _IDE_PORT_CONTROL, 0x80 | channels[secondary].noINT);
	if (reg < 0x08)
		result = inb(channels[secondary].base + reg - 0x00);
	else if (reg < 0x0C)
		result = inb(channels[secondary].base  + reg - 0x06);
	else if (reg < 0x0E)
		result = inb(channels[secondary].control  + reg - 0x0A);
	else if (reg < 0x16)
		result = inb(channels[secondary].busmasteride + reg - 0x0E);
	if (reg > 0x07 && reg < 0x0C)
		_IDE_writePort(secondary, _IDE_PORT_CONTROL, channels[secondary].noINT);
	return result;
...
void _IDE_writePort(bool secondary, uint8_t reg, uint8_t data)
{
	if (reg > 0x07 && reg < 0x0C)
		_IDE_writePort(secondary, _IDE_PORT_CONTROL, 0x80 | channels[secondary].noINT);
	if (reg < 0x08)
		outb(channels[secondary].base  + reg - 0x00, data);
	else if (reg < 0x0C)
		outb(channels[secondary].base  + reg - 0x06, data);
	else if (reg < 0x0E)
		outb(channels[secondary].control  + reg - 0x0A, data);
	else if (reg < 0x16)
		outb(channels[secondary].busmasteride + reg - 0x0E, data);
	if (reg > 0x07 && reg < 0x0C)
		_IDE_writePort(secondary, _IDE_PORT_CONTROL, channels[secondary].noINT);
	/*uint16_t base = _IDE_PRIMARY_PORT_BASE;
	if (secondary)
...
void _IDE_init()
{
	...
	channels[0].base = (_IDE_PRIMARY_PORT_BASE & 0xFFFFFFFC) + _IDE_PRIMARY_PORT_BASE * (!_IDE_PRIMARY_PORT_BASE);;
	channels[0].control = (_IDE_PRIMARY_PORT_NOINT & 0xFFFFFFFC) + _IDE_PRIMARY_PORT_NOINT * (!_IDE_PRIMARY_PORT_NOINT);;
	channels[0].busmasteride = (0 & 0xFFFFFFFC) + 0;
	channels[1].base = (_IDE_SECONDARY_PORT_BASE & 0xFFFFFFFC) + _IDE_SECONDARY_PORT_BASE * (!_IDE_SECONDARY_PORT_BASE);
	channels[1].control = (_IDE_SECONDARY_PORT_NOINT & 0xFFFFFFFC) + _IDE_SECONDARY_PORT_NOINT * (!_IDE_SECONDARY_PORT_NOINT);
	channels[1].busmasteride = (0 & 0xFFFFFFFC) + 8;
